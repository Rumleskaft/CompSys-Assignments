...
cache_ptr create_cache(const char* name, int block_bits, int index_bits, int assoc,
		       int hit_latency, cache_ptr next_cache, int miss_latency) {
    if (block_bits < 3)
...
static unsigned long* cache_op(cache_ptr cache, unsigned long earliest, 
			       unsigned long addr, bool* triggered_miss) {
	...
	data_ready = cache_read(cache->lower_level, earliest, addr, NULL);
    }
...
unsigned long cache_read(cache_ptr cache, unsigned long earliest, unsigned long addr, bool* triggered_miss) {
    unsigned long* timing_info = cache_op(cache, earliest, addr, triggered_miss);
    cache->reads++;
    return time_read(cache, earliest, *timing_info);
}

unsigned long cache_write(cache_ptr cache, unsigned long earliest, unsigned long addr, bool* triggered_miss) {
    unsigned long* timing_info = cache_op(cache, earliest, addr, triggered_miss);
